...
exports.list = async (req, res) => {
  ...
  const data = ops.map((o) => ({
    ...
    supplierName: o.supplierOrCustomer,
    destLocation: o.destLocation ? { id: o.destLocation.id, name: o.destLocation.name } : null,
...
exports.create = async (req, res) => {
  const { supplierName, destLocationId, scheduledDate, lines } = req.body;
  const referenceNo = await generateReferenceNo('RECEIPT');
  const op = await prisma.operation.create({
    ...
    include: { lines: { include: { product: true } }, destLocation: true },
  });
  res.status(201).json(formatReceipt(op));
};
...
exports.update = async (req, res) => {
  ...
  res.json(formatReceipt(updated[1]));
};
...
exports.cancel = async (req, res) => {
  ...
  res.json({ message: 'Receipt cancelled', status: 'cancelled' });
};

function formatReceipt(op) {
  return {
    id: op.id,
    referenceNo: op.referenceNo,
    status: op.status.toLowerCase(),
    supplierName: op.supplierOrCustomer,
    destLocation: op.destLocation ? { id: op.destLocation.id, name: op.destLocation.name } : null,
